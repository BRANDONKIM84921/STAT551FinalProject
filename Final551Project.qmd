...
We want to improve our decision tree that predicts type of crime by making a boosted tree model and we want to tune cost and tree depth. We will also find further metrics for our model. We may also want to use more years but we have a lot of data between 2022-2023 so this many be sufficient and may actually be preferred because crime types differ over years. Lastly, we made a predictive boosted model that classifies if an arrest was made or not based on a set of predictors. It may also be useful to make an interpretable logistic model that will allow us to see which predictors impact arrests being made.
