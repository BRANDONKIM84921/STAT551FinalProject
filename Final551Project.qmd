...
animatedplot <- ggplot(new_crimes_w_clusters, aes(x = `X Coordinate`, y = `Y Coordinate`, group = interaction(Year, `X Coordinate`), color=as.factor(cluster))) +
  ...
  labs(color = "Cluster", 
       ...
       title = "Many Crimes on X and Y Coordinates by Year with K Means Clusters") 
...
graph1 <- merged_df %>% ggplot()+
  ...
  scale_color_manual(name = "Cluster", values = c("blue", "orange", "green", "pink","red")) +
    labs(color="Cluster")+
  guides(color = "legend")+
...
graph1.animation = graph1 +
  theme_minimal()+
  geom_text(aes(x=mean_x_coordinate,y=mean_y_coordinate, label = paste("Arrest Rate: ", round(arrest_rate, 2) ), color = ifelse(arrest_rate > 0.45, "Above 0.45", "Below or equal to 0.45"), group = interaction(year, mean_x_coordinate)), show.legend = TRUE) +
    scale_color_manual(name = "Arrest Rate", values = c("hotpink", "purple"))+
  transition_time(year) +
    guides(color = "legend")+

  labs(subtitle = "Year: {str_sub(frame_time, 1, 4)}", color="Arrest Rate", title="Homicides in Chicago by Year with KMeans Clusters")
...
```{r}
graph1 <- ggplot(merged_df, aes(x = `X Coordinate`, y = `Y Coordinate`, group = interaction(Year, `X Coordinate`), color=as.factor(cluster)))+
  geom_point(alpha=0.15)+
  labs(color="Cluster", title="Criminal Sexual Assaults in Chicago by Year with KMeans Clusters")+
  xlim(1110000, 1225000)
...
graph1.animation = graph1 +
  theme_minimal()+
  geom_text(aes(x=mean_x_coordinate,y=mean_y_coordinate, label = paste("Arrest Rate: ", round(arrest_rate, 2) ), color = ifelse(arrest_rate > 0.2, "Above 0.2", "Below or equal to 0.2")), show.legend = FALSE) +
  transition_time(Year) +
  labs(subtitle = "Year: {str_sub(frame_time, 1, 4)}")
...
graph1 <- ggplot(merged_df, aes(x = `X Coordinate`, y = `Y Coordinate`, group = interaction(Year, `X Coordinate`), color=as.factor(cluster)))+
  geom_point(alpha=0.15)+
  labs(color="Cluster", title="Interference with Public Officer in Chicago by Year with KMeans Clusters")+
  xlim(1110000, 1225000)
...
graph1.animation = graph1 +
  theme_minimal()+
  geom_text(aes(x=mean_x_coordinate,y=mean_y_coordinate, label = paste("Arrest Rate: ", round(arrest_rate, 2) ), color = ifelse(arrest_rate > 0.9, "Above 0.9", "Below or equal to 0.9")), show.legend = FALSE) +
  transition_time(Year) +
  labs(subtitle = "Year: {str_sub(frame_time, 1, 4)}")
...
animate(graph1.animation, height = 500, width = 800, fps = 30, duration = 30,
        end_pause = 60, res = 100)
anim_save("interferencewPO_cluster_analysis.gif")
