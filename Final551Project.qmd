...
- Using multiple non-supervised clustering methods of varying precision to determine if specific locations, based on latitude and longitude, and times are associated with the frequency of certain crimes. (Particularly about homicides, sexual assault and Officer Interference)
...
data.frame(table(crimes$`Primary Type`), stringsAsFactors = FALSE) %>%
  ...
  layout(title = "Frequency of Incidents in Chicago by Type of Crime <br>
         <sup>Recorded by City of Chicago between January 1st, 2001 and November 7th, 2023</sup>",
...
crimes %>% 
  ...
  layout(title = "Frequency of Incidents in Chicago by Year <br>
         <sup>Recorded by City of Chicago between January 1st, 2001 and November 7th, 2023</sup>", 
         xaxis = list(title = "Number of Incidents (Total Crimes of the types in the previous plot)"),
         yaxis = list(title = ""))
```
